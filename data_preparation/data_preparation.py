...
def download_OfficeHome(args):
    ...
    (args.dataset_dir / 'OfficeHome' / 'RealWorld').rename(args.dataset_dir / 'OfficeHome' / 'Real')
...
def prepare_text_list(args):
    ...
    for (types, output_type), domain, num_labels in product(
        ...
        download(
            ...
            args.dataset_dir / args.dataset / 'text' / domain / f'{output_type}_{num_labels}.txt',
        )
...
def arguments_parsing():
    ...
    p.add('--dataset', type=str, default='OfficeHome', choices=['DomainNet', 'Office31', 'OfficeHome'])
